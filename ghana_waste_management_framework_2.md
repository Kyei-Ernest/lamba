...
## 4. Sizing and Pricing Strategy (Polythene Bags vs. Standard Bins)

To prevent customer disputes and protect collectors from carrying unfeasibly heavy/bulky waste for a flat "per bag" fee, pricing must be strictly standardized by **volume or physical bin size**.

### A. Standard Bin Sizing (For Monthly/Scheduled Subscriptions)
For households with standard plastic bins on wheels, pricing is calculated according to official municipal capacities:
*   **120-Liter Bin (Standard Small):** Best for single families. Charged at a base rate (e.g., GHS 25 per pickup).
*   **240-Liter Bin (Standard Medium):** The most common bin size in Ghanaian households. Charged at GHS 40 per pickup.
*   **360-Liter Bin (Large/Compound Houses):** Often shared by multiple tenants. Charged at GHS 60 per pickup.

### B. Polythene Sizing Standards (For On-Demand "Pay-As-You-Throw")
Since many households in informal or low-income suburbs do not own plastic bins, they package waste in black polythene bags bought from markets. To prevent customers from packing massive charcoal or cocoa sacks (which can weigh up to 50kg) and expecting to pay a single "bag" rate, we establish clear thresholds:
*   **Standard Size (up to 40 Liters):** Equivalent to a normal domestic black bin liner. Base charge: GHS 10 per bag.
*   **Jumbo Size (up to 80 Liters):** Large heavy-duty garden waste bags. Base charge: GHS 20 per bag.
*   **Sack/Bulky (Over 80 Liters):** Fertilizer/cocoa/jute sacks. These require special lifting and vehicle space and are priced at GHS 35+ per sack.

```
┌────────────────────────────────────────────────────────────────────────────┐
│                    PRICING AND CONTAINER SIZING MATRIX                     │
├──────────────────────┬──────────────────────┬──────────────────────────────┤
│ Container Type       │ Standard Capacity    │ Target Base Charge (GHS)     │
├──────────────────────┼──────────────────────┼──────────────────────────────┤
│ Standard Poly Bag    │ Up to 40 Liters      │ GHS 10.00                    │
│ Jumbo Poly Bag       │ Up to 80 Liters      │ GHS 20.00                    │
│ 120L Wheelie Bin     │ 120 Liters           │ GHS 25.00                    │
│ 240L Wheelie Bin     │ 240 Liters           │ GHS 40.00                    │
│ Bulk Cocoa/Jute Sack │ 100+ Liters          │ GHS 35.00+ (Special rate)    │
└──────────────────────┴──────────────────────┴──────────────────────────────┘
```

### C. The "Prepaid Branded Bag" Model (Ultimate Low-Tech Pricing)
The most realistic and friction-free pricing method is to sell **prepaid branded biodegradable bags** (e.g., green for organics/mixed, yellow for plastic recyclables) through local shops, tabletop vendors, and supermarkets:
1.  **Buy the Bag:** The customer buys a pack of 5 "Borla Tracker Bags" for GHS 50 (GHS 10 per bag).
2.  **No App/No MoMo Needed at Pickup:** The purchase price of the bag *includes* the waste collection service fee. The customer doesn't need to request a pickup or pay anything at the gate.
3.  **Automatic Collector Incentive:** The bag has a unique printed code (or a simple perforated token label). The collector picks up only these branded bags, enters the code via USSD (or collects the physical token), and instantly receives GHS 7 in their digital wallet. 
4.  **Why this works:** It removes all payment arguments, requires zero digital interaction from the homeowner at collection time, and guarantees that collectors only pick up standardized, manageable volumes.

---

## 5. Summary of the Adaptable Workflow

1.  **Request:** A resident in Ashaiman buys a GHS 10 "Borla Tracker Bag" from a nearby corner shop.
2.  **Dispatch:** When the bag is full, they place it outside. An automated IVR call or USSD notification alerts nearby collectors of a "Borla Bag" presence in their zone.
3.  **Collection:** Kwesi (the *Aboboyaa* rider) spots the bag, picks it up, and tears off the perforated code ticket attached to the handle.
4.  **Verification:** Kwesi dials `*800#` on his feature phone, enters the bag's unique printed code, and GHS 7 is instantly credited to his wallet.
5.  **Disposal:** Kwesi dumps the waste at a registered transfer station for free (tipping fee is cleared by the platform from the remaining GHS 3).
6.  **Payout:** Kwesi transfers his day's earnings to his MTN MoMo account instantly to cover fuel and maintenance.
